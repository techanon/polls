...
class API extends \Restful {
	...
	public function get_poll($id) {
		$force = (isset($_GET['force']) && $_GET['force'] == true) ? true : false;
		if ($id > 0) {
			$p = new Poll ($id);
			if ($p && !$p->visible) {
		...
		$dovote = User::require_login() ? ((array_sum($o->res->mine) > 0 && !isset($_GET['vote'])) || $force ) ? false : true : false;
